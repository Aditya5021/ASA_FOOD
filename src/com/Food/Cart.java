		...
		{					  
			...
			while(resultSet.next())
			{
				...
				if(item_id.equals(compare))
				{
					...
					sql="update cart set cart_ord_qty=cart_ord_qty+? where item_id=? and cust_email=? and qtyno=?";
					ps = con.prepareStatement(sql);
					ps.setString(1,qty);
					ps.setString(2,item_id);
					ps.setString(3,cust_email);
					if(qty.equals("Large")) {ps.setString(4,"2");}
					else if(qty.equals("Medium")) {ps.setString(4,"1.5");}
					else if(qty.equals("Small")) {ps.setString(4,"1");}
			...
			if(f==0)
			{
				sql ="insert into cart(item_id,cust_email,rest_id,cart_ord_qty,qtyno) values (?, ?, ?,?,?)";
				item_count++;
				...
				s.setString(4,qty);
				if(qty.equals("Large")) {s.setString(5,"3");}
				else if(qty.equals("Medium")) {s.setString(5,"2");}
				else if(qty.equals("Small")) {s.setString(5,"1");}
				s.executeUpdate();
